...
function initRender() {
    renderer = new THREE.WebGLRenderer({
        antialias: true   // 开启抗锯齿。如果图片和显示器的分辨率出现差错，则可能导致图片拉伸，产生不好的效果；若开启抗锯齿，则会重新计算比例，进而改善画质，但是会有额外的性能开销。
    });
...
function initModel() {
    ...
    loader.load('./source/Naruto.fbx', (mesh) => { // 模型加载需要时间
        ...
        function createAction(i) {
            actions[i] = mixer.clipAction(mesh.animations[i]); // 原生方法，传入一个动画对象，返回一个可用对象
            // actions[i].play();
            // 调用动画， 播放
...
function animate() {
    render() // 每一帧都执行render方法进行渲染，否则页面中的人物会一动不动站在那（action24）
